...
async function bonfireStart(input_eth){
	...
	//estimate gasLimit
	var encodedData = tokenInst.methods.bonfireEvent(deadline).encodeABI();
	...
	//transaction
	tokenInst.methods.bonfireEvent(deadline).send({
		from: MyLibrary.wallet,
		value: input_eth,
   		gasPrice: gasPrice,
...
async function port_rewardsAvail(){
	...
	var rewardsEth = parseFloat(rewardsEth).toFixed(4);//float so we can add the values not append
	if(rewardsEth > 0){
...
$(document).on('click', '#playRR', function(e){
	...
	if($('#RRwormhole').css('height')<'50px'){
		...
		//change button
		document.getElementById("playRR").innerHTML = "<span>close</span>";
		...
		//restore button
		document.getElementById("playRR").innerHTML = "<span>coming soon</span>";
